...
export default async function SettingsPage() {
  ...
  return (
    <AppShell active="/admin/settings">
      <h1 style={{ fontSize: 26, marginBottom: 16 }}>Settings</h1>

      <div className="card" style={{ marginBottom: 16 }}>
        <div className="hd"><h2 style={{ fontSize: 16 }}>Company — {company.name}</h2></div>
        <div className="bd">
          <form action={updateCompanyAction}>
            ...
            <button className="btn pri" type="submit">Save company details</button>
          </form>
        </div>
      </div>

      <div className="card" style={{ marginBottom: 16 }}>
        <div className="hd"><h2 style={{ fontSize: 16 }}>CPF rates</h2><span className="hint">Editable in case MOM/CPF Board rates change — defaults are the January 2026 table.</span></div>
        <div className="bd">
          <form action={updateCpfAction}>
            <input type="hidden" name="id" value={company.id} />
            <div className="grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 14 }}>
            ...
            <div className="hint" style={{ margin: "6px 0" }}>Each pair is Total % / Employee %.</div>
            <button className="btn pri" type="submit">Save CPF rates</button>
          </form>
        </div>
      </div>

      <div className="card" style={{ marginBottom: 16 }}>
        <div className="hd"><h2 style={{ fontSize: 16 }}>Foreign worker levy tiers</h2></div>
        <div className="bd">
          {levies.map((l) => (
            <form key={l.id} action={updateLevyAction} className="flex items-center gap-2" style={{ padding: "6px 0", borderBottom: "1px solid var(--line)" }}>
          ...
          <form action={createLevyAction} className="flex items-center gap-2" style={{ marginTop: 10 }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="hd"><h2 style={{ fontSize: 16 }}>Companies</h2></div>
        <div className="bd">
          <table>
            <tbody>
              {companies.map((c) => (
          ...
          <form action={addCompanyAction} className="flex items-center gap-2" style={{ marginTop: 12 }}>
      ...
      <div className="card">
        <div className="hd"><h2 style={{ fontSize: 16 }}>Users &amp; logins</h2><span className="hint">Create a login for your colleague so they can key in MC/leave. Give them the Staff role — only Administrators can see payroll.</span></div>
        <div className="bd">
